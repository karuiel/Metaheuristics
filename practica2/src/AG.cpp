#include "AG.h"
	...
	AG::AG(vector<Example> &train,vector<vector<float>>poblacionInicial){
		particionTrain=train;
		Cromosoma auxcromosoma;
		llamadas=1000;
		indiceMejorPoblacion=0;
		for(int i=0;i<poblacionInicial.size();i++){
			auxcromosoma.pesos=poblacionInicial[i];
			poblacion.push_back(auxcromosoma);
			poblacion[i].porcentaje=poblacion[i].KNN(train);
			llamadas--;
		}
	...
	void AG::seleccion(int numIteraciones){
		...
		for(int i=0;i<numIteraciones;i++){
			indiceCromosoma1=rand()%10;
			indiceCromosoma2=rand()%10;
	...
	vector<float> AG::AGGBLX(int iteraciones){
